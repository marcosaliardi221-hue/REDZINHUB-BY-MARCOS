...
local function LoadScript(tier, key)
    ...
    if not url then
        warn("[MARK HUB] No " .. tier .. " script for GameId: " .. tostring(gameId))
        return
    end
    ...
    if not ok then warn("[MARK HUB] Error: " .. tostring(err)) end
end
